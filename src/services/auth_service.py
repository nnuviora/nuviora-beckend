...
class AuthService(Protocol):
    def __init__(self, user_repo, cache_manager, email_manager, security_layer) -> None:
        ...
        try:
            if data["auth_type"] == "login":
                ...
                email = await self.email_manager.send_email(
                    recipient=data["email"], subject=None
                )
                return {"message": None}
    ...
    async def verify_handler(self, token: str) -> dict:
        try:
            ...
            access_token, refresh_token = asyncio.gather(
                self.create_access_token(), self.create_refresh_token()
            )
        ...
        try:
            ...
            if not (
                await self.security_layer.verify_password(
                    password=data["password"], hash_password=obj["hash_password"]
                )
            ):
                return None  # Warning
            access_token, refresh_token = asyncio.gather(
